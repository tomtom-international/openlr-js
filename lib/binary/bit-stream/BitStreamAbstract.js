...
class BitStreamAbstract {
    ...
    _expandBuffer(newLength) {
        ...
        if (normalizedNewLength > this._totalBufferLengthBytes) {
            ...
            const newBuffer = buffer_1.Buffer.alloc(normalizedNewLength);
    ...
    _createBuffer(length) {
        ...
        this._buffer = buffer_1.Buffer.alloc(length);
    }
    _getData() {
        const byteLength = this._currentBit >>> BitStreamAbstract._BIT_BYTE_SHIFT;
        const copyBuffer = buffer_1.Buffer.alloc(byteLength);
...
;
//# sourceMappingURL=BitStreamAbstract.js.map