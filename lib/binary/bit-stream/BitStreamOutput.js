...
export default class BitStreamOutput extends BitStreamAbstract {
    ...
    putBits(value, countBitsToPut) {
        ...
        let endByteIndex = (this._currentBit + countBitsToPut - 1) >>> BitStreamAbstract._BIT_BYTE_SHIFT;
        const beginByteIndex = this._currentBit >>> BitStreamAbstract._BIT_BYTE_SHIFT;
        const freeBitsFirstByte = BitStreamAbstract._BYTE_SIZE - (this._currentBit % BitStreamAbstract._BYTE_SIZE);
        ...
        if (freeBitsFirstByte >= countBitsToPut) {
            ...
            const countBitsToPutLastByte = (countBitsToPut - freeBitsFirstByte) % BitStreamAbstract._BYTE_SIZE;
            if (countBitsToPutLastByte > 0) {
                this._buffer[endByteIndex] = 0;
                this._buffer[endByteIndex] |= (remainingValue << BitStreamAbstract._BYTE_SIZE - countBitsToPutLastByte) & BitStreamAbstract._BITMASK[BitStreamAbstract._BYTE_SIZE];
                remainingValue >>= countBitsToPutLastByte;
                endByteIndex--;
            }
            for (; endByteIndex > beginByteIndex; endByteIndex--) {
                this._buffer[endByteIndex] = 0;
                this._buffer[endByteIndex] |= remainingValue & BitStreamAbstract._BITMASK[BitStreamAbstract._BYTE_SIZE];
